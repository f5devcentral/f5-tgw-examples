...
create /net tunnels tunnel tgw-connect remote-address 10.254.254.11 traffic-group traffic-group-local-only profile gre local-address ${var.bigip1_self}
...
modify sys db config.allow.rfc3927 { value "enable" }
modify sys db connection.vlankeyed value disable
create /net tunnels tunnel tgw-connect remote-address 10.254.254.12 traffic-group traffic-group-local-only profile gre local-address ${var.bigip2_self}
...
#   create /net route tgw network 10.0.0.0/8 gw 10.1.7.1
#   create /net route tgw_connect network 10.254.254.0/24 gw 10.1.7.1
# create /net tunnels tunnel tgw-connect remote-address 10.254.254.13 traffic-group traffic-group-local-only profile gre local-address ${var.bigip3_self}
...
# modify sys db config.allow.rfc3927 { value "enable" }
# modify sys db connection.vlankeyed value disable
# create /net tunnels tunnel tgw-connect remote-address 10.254.254.14 traffic-group traffic-group-local-only profile gre local-address ${var.bigip4_self}
